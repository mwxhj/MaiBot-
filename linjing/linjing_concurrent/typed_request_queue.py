...
class TypedRequestQueue(Generic[T, R]):
    ...
    async def _process_request(self, task: RequestTask[T, R]) -> None:
        ...
        try:
            ...
            logger.warning(f"任务 {task.request_id} 被取消 (可能由于超时)。") # 使用 logger
            task.mark_failed(TimeoutError("Task timed out and was cancelled"))
        except Exception as e:
            logger.error(f"处理任务 {task.request_id} 时发生错误: {e}", exc_info=True) # 使用 logger
            task.mark_failed(e)
        finally:
            # 减少活跃任务计数器
            if self.active_tasks > 0: # 避免减到负数
                 self.active_tasks -= 1
            logger.debug(f"任务 {task.request_id} 处理结束，当前活跃任务数: {self.active_tasks}") # 使用 logger
            # 尝试处理下一个任务
